...
int main(int argc, char **argv)
{
	...
	toSend =
	    dbus_message_new_method_call(POTATO_DESTINATION, batDev,
					 POTATO_CALL_INTERFACE,
					 POTATO_CALL_METHOD);
	...
	// send the message
	reply =
	    dbus_connection_send_with_reply_and_block(conn, toSend,
						      POTATO_TIMEOUT, &err);
	if (reply == NULL) {
		fprintf(stderr, "%s: %s\n", err.name, err.message);
